...
def build_local_cluster(
    ...
    launcher = build_local_cluster_launch_script(
        results_dir=results_dir,
        diagnostics_dir=diagnostics_dir,
        input_data=input_data,
    )
    
    subprocess.run(["bash", launcher.name])
...
def build_cluster_launch_script(
    ...
    worker_bindings = get_worker_bindings(input_data, results_dir, diagnostics_dir)
...
def build_local_cluster_launch_script(
    results_dir: Path, diagnostics_dir: Path, input_data: List[Path]
) -> TextIO:
    """Generates a shell file that, on execution, spins up a Spark cluster.
...
singularity exec instance://master \
    $CONDA_PATH run --no-capture-output -n $CONDA_ENV \
    $SPARK_ROOT/bin/spark-class org.apache.spark.deploy.master.Master \
    --host $SPARK_MASTER_HOST \
    --port $SPARK_MASTER_PORT \
    --webui-port $SPARK_MASTER_WEBUI_PORT \
        > /tmp/spark_cluster_$USER/spark_master.log 2>&1 &

singularity exec instance://worker \
    $CONDA_PATH run --no-capture-output -n $CONDA_ENV \
    $SPARK_ROOT/bin/spark-class org.apache.spark.deploy.worker.Worker \
    --cores 1 --memory 512M \
    --webui-port $SPARK_WORKER_WEBUI_PORT \
...
def get_worker_bindings(input_data: List[Path], results_dir: Path, diagnostics_dir: Path) -> str:
